...
let allCriminals = () => { 
    //Everyone with two or more of the letter t in their full name is a criminal!
    let rInLNameInnocent = /.+\..*r.*@/i;
    let cInNameCoUkendCriminal = /.*c.*@.+\.co\.uk/i;
    let grinOrWonkFName4PlusCriminal = /.{4}\..*@(gringottsbank|wonkaindustries)\./i;
    let name2plusTCriminal = /(.*tt.*|.*t.*t.*)@/i;
    
    return emails.filter(i =>
        !rInLNameInnocent.test(i) &&
        !(
            cInNameCoUkendCriminal.test(i) ||
            grinOrWonkFName4PlusCriminal.test(i) ||
            name2plusTCriminal.test(i))
    );
}
